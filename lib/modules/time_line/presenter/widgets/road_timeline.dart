...
class _RoadTimelineState extends State<RoadTimeline> {
  ...
  void didUpdateWidget(RoadTimeline oldWidget) {
    ...
    if (!identical(oldWidget.moments,       widget.moments) ||
        !identical(oldWidget.timelineNames, widget.timelineNames)) {
  ...
  double _measure(String text, TextStyle style, int maxLines, double maxW) =>
      (TextPainter(
  ...
  static TextStyle _titleStyle(TextTheme t) =>
  ...
  static TextStyle _subStyle(TextTheme t) =>
  ...
  double _perspScale(double vy, double sH) =>
      0.75 + (vy / sH).clamp(0.0, 1.0) * 0.35;
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < moments.length; i++) {
      final m          = moments[i];
      final pinColor   = widget.timelineColors[m.timelineId]
          ?? m.type.colors(context).bg;
      final tn         = widget.timelineNames[m.timelineId] ?? '';
      ...
      final subtitle = tn.isNotEmpty
          ? '$tn • ${m.dateTimeFormatted}'
          : m.dateTimeFormatted;
        ...
        stack.add(Positioned(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border(
                left:  dOnLeft ? BorderSide.none
                               : BorderSide(color: pinColor, width: 3),
                right: dOnLeft ? BorderSide(color: pinColor, width: 3)
                               : BorderSide.none,
              ),
              boxShadow: [
                BoxShadow(
    ...
    if (_selectedIndex != null && _selectedIndex! < moments.length) {
      ...
      final cardTop  = arrowBot
          ? pinTop - estCardH - _arrowH - 2
          : cy + _pinH / 2 + _arrowH + 2;
      ...
      child: SizedBox(
        ...
        child: Stack(
          ...
          children: [
            ...
            Positioned(
              top: 8,
              child: Icon(moment.type.icon, size: 16, color: Colors.white),
            ),
          ],
        ),
      ),
...
class _FloatingCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border(top: BorderSide(
                    color: pinColor.withValues(alpha: 0.15), width: 1)),
              ),
              child: Row(
    ...
    final arrow = SizedBox(
      width: cardW, height: arrowH,
      child: CustomPaint(
        painter: _ArrowPainter(
          color:     pinColor,
          arrowX:    arrowX,
          arrowW:    arrowW,
          pointDown: arrowBelow,
        ),
      ),
    );
...
class _ArrowPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(_tri(x, half,       size.height, 0),
        Paint()..color = color);
    canvas.drawPath(_tri(x, half - 1.5, size.height, 1.5),
  ...
  Path _tri(double x, double hw, double h, double inset) {
    final p = Path();
    if (pointDown) {
      p..moveTo(x - hw, inset)..lineTo(x + hw, inset)..lineTo(x, h - inset);
    } else {
      p..moveTo(x - hw, h - inset)..lineTo(x + hw, h - inset)..lineTo(x, inset);
    }
    return p..close();
  }

  @override
  bool shouldRepaint(covariant _ArrowPainter o) =>
      o.color != color || o.arrowX != arrowX || o.pointDown != pointDown;
}
            ...
            Container(
              ...
              decoration: BoxDecoration(
                gradient: const LinearGradient(
                  colors: [Color(0xFFEFD6F7), Color(0xFFFBDFEB)],
                ),
...
class _RoadPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var li = 0; li < 5; li++) {
      canvas.drawPath(
        _strip(li / 5.0, (li + 1) / 5.0, w, h),
        Paint()..color = _laneColors[li],
      );
    }
